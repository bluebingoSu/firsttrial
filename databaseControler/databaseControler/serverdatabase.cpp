...
bool ServerDataBase::connectDataBase(QString connectName){
...
bool ServerDataBase::getAllUserView(QList<QJsonObject> &jsonUserViewList){
    if(!query.exec("SELECT * FROM user_view")){
        qDebug() << "Failed to select from user_view: " << query.lastError();
        return false;
    }
    while(query.next()){
        ...
        jsonUserView["avatar"] = query.value("avatar").toString();
        jsonUserView["id"] = query.value("id").toString();
        jsonUserView["ip"] = query.value("ip").toString();
        jsonUserView["name"] = query.value("name").toString();
        jsonUserView["department"] = query.value("department").toString();
        jsonUserView["online"] = query.value("department").toInt();
        jsonUserViewList.append(jsonUserView);

    }
    qDebug() << "Succeed to select all from user_view!";
    return true;

}

void ServerDataBase::changeJsonIntoUser(QString &avatar, QString &id, QString &ip, QString &name, QString &department, bool &online, QJsonObject jsonUserView){
    avatar=jsonUserView["avatar"].toString();
    id=jsonUserView["id"].toString();
    ip=jsonUserView["ip"].toString();
    name=jsonUserView["name"].toString();
    department=jsonUserView["department"].toString();
    online=jsonUserView["online"].toInt();
}
...
void ServerDataBase::changeMessageIntoJson(QString idSender, QString idReceiver, QString timestamp, QString type, QString data, QJsonObject &jsonMessage){
    jsonMessage["idSender"] = idSender;
    jsonMessage["idReceiver"] = idReceiver;
    jsonMessage["timestamp"] = timestamp;
    jsonMessage["type"] = type;
    jsonMessage["data"] = data;
}

void ServerDataBase::changeJsonIntoMessage(QString &idSender, QString &idReceiver, QString &timestamp, QString &type, QString &data, QJsonObject jsonMessage){
    idSender = jsonMessage["idSender"].toString();
    idReceiver = jsonMessage["idReceiver"].toString();
    timestamp = jsonMessage["timestamp"].toString();
    type = jsonMessage["type"].toString();
    data = jsonMessage["data"].toString();
}

bool ServerDataBase::getAllMessage(QList<QJsonObject> &jsonMessageList){
    if(!query.exec("SELECT * FROM message ORDER BY timestamp DESC")){
        qDebug() << "Failed to select the message: " << query.lastError();
        return false;
    }
    while(query.next()){
    ...
    qDebug() << "Succeed to get all the message!";
    return true;
}

bool ServerDataBase::selectMessageByData(QString idSender, QString idReceiver, QString dataPart, QList<QJsonObject> &jsonMessageList){
    query.prepare("SELECT * FROM message WHERE idSender = ? AND idReceiver = ? AND data LIKE ? ORDER BY timestamp");
    QVariantList a,b,c;
    a << idSender;
    b << idReceiver;
    c << "%" + dataPart + "%";
    ...
    query.addBindValue(c);
    if(!query.execBatch()){
        qDebug() << "Failed to select the message by data: " << query.lastError();
        return false;
    }
    if(!query.next()){
        qDebug() << "Failed to find the message by data: " << query.lastError();
        return false;
    }
    do{
        QJsonObject jsonMessage;
    ...
    qDebug() << "Succeed to select the message by data!";
    return true;
}
