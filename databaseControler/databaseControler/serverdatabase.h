...
public:
    ServerDataBase();
    bool connectDataBase(QString connectName);//连接数据库
    ...
    bool getAllUserView(QList<QJsonObject> &jsonUserViewList);//获得所有用户
    void changeJsonIntoUser(QString &avatar,QString &id,QString &ip,QString &name,QString &department,bool &online,QJsonObject jsonUserView);//将json转为用户
    ...
    void changeMessageIntoJson(QString idSender,QString idReceiver,QString timestamp,QString type,QString data,QJsonObject &jsonMessage);//将消息转为json
    void changeJsonIntoMessage(QString &idSender,QString &idReceiver,QString &timestamp,QString &type,QString &data,QJsonObject jsonMessage);//将json转为消息
    bool getAllMessage(QList<QJsonObject> &jsonMessageList);//获得所有消息
    bool selectMessageByData(QString idSender, QString idReceiver, QString dataPart, QList<QJsonObject> &jsonMessageList);//服务端查询消息记录
